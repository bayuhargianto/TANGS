...
class C_tipe_karyawan extends MY_Controller {
	private $any_error = array();
	// Define Main Table
	public $tbl = 'm_type_karyawan';
	...
	public function view(){
		...
		$data = array(
			...
			'title_page' 	=> 'Tipe Karyawan',
			'title_page2' 	=> 'Master Tipe Karyawan',
			'priv_add'		=> $priv['create']
			);
		if($priv['read'] == 1)
		{
			$this->open_page('tipe-karyawan/V_tipe_karyawan', $data);
		}
		else
		{
		...
		if ($query<>false) {
			...
			foreach ($query->result() as $val) {
				...
				if ($val->type_karyawan_status_aktif == 'y') {
					...
					if($priv['update'] == 1)
					{
						$button = $button.'<button class="btn blue-ebonyclay" type="button" onclick="openFormTipeKaryawan('.$val->type_karyawan_id.')" title="Edit" data-toggle="modal" href="#modaladd">
											<i class="icon-pencil text-center"></i>
										</button>';
					}
					if($priv['delete'] == 1)
					{
						$button = $button.'<button class="btn red-thunderbird" type="button" onclick="deleteData('.$val->type_karyawan_id.')" title="Non Aktifkan">
					...
					if($priv['update'] == 1)
					{
						$button = $button.'<button class="btn blue-ebonyclay" type="button" onclick="openFormTipeKaryawan('.$val->type_karyawan_id.')" title="Edit" data-toggle="modal" href="#modaladd" disabled>
											<i class="icon-pencil text-center"></i>
										</button>';
					}
					if($priv['delete'] == 1)
					{
						$button = $button.'<button class="btn green-jungle" type="button" onclick="aktifData('.$val->type_karyawan_id.')" title="Aktifkan">
						<i class="icon-power text-center"></i>
					</button>';
					}
	...
	public function getForm(){
		$this->load->view("tipe-karyawan/V_form_tipe_karyawan");
	}
	...
	public function loadDataWhere(){
		...
		if ($query<>false) {
			...
			foreach ($query->result() as $val) {
				$response['val'][] = array(
					'kode' 							=> $val->type_karyawan_id,
					'type_karyawan_nama' 			=> $val->type_karyawan_nama,
					'type_karyawan_status_aktif' 	=> $val->type_karyawan_status_aktif
				);
			}
	...
	public function loadData_select(){
		...
		$select = '*';
		$where['data'][] = array(
			'column' => 'type_karyawan_status_aktif',
			'param'	 => 'y'
		);
		$where_like['data'][] = array(
			'column' => 'type_karyawan_nama',
			'param'	 => $this->input->get('q')
		);
		$order['data'][] = array(
			'column' => 'type_karyawan_nama',
		...
		if ($query<>false) {
			foreach ($query->result() as $val) {
				$response['items'][] = array(
					'id'	=> $val->type_karyawan_id,
					'text'	=> $val->type_karyawan_nama
				);
			}
			$response['status'] = '200';
		}
	...
	public function postData(){
		...
		if (strlen($id)>0) {
			...
			if($data['type_karyawan_status_aktif'] == 'n')
			{
	...
	function general_post_data($type, $id = null){
		...
		$queryRevised = $this->mod->select('type_karyawan_revised', $this->tbl, NULL, $where);
		if ($queryRevised) {
			$revised = $queryRevised->row_array();
			$rev = $revised['type_karyawan_revised'] + 1;
		}
		if ($type == 1) {
			$data = array(
				'type_karyawan_nama' 			=> $this->input->post('type_karyawan_nama', TRUE),
				'type_karyawan_status_aktif' 	=> $this->input->post('type_karyawan_status_aktif', TRUE),
				'type_karyawan_create_date' 	=> date('Y-m-d H:i:s'),
				'type_karyawan_update_date' 	=> date('Y-m-d H:i:s'),
				'type_karyawan_create_by' 		=> $this->session->userdata('user_username'),
				'type_karyawan_revised' 		=> 0,
			);
		} else if ($type == 2) {
			$data = array(
				'type_karyawan_nama' 			=> $this->input->post('type_karyawan_nama', TRUE),
				'type_karyawan_status_aktif' 	=> $this->input->post('type_karyawan_status_aktif', TRUE),
				'type_karyawan_update_date' 	=> date('Y-m-d H:i:s'),
				'type_karyawan_update_by' 		=> $this->session->userdata('user_username'),
				'type_karyawan_revised' 		=> $rev,
			);
		} else if ($type == 3) {
			$data = array(
				'type_karyawan_status_aktif' 	=> 'n',
				'type_karyawan_update_date' 	=> date('Y-m-d H:i:s'),
				'type_karyawan_update_by' 		=> $this->session->userdata('user_username'),
				'type_karyawan_revised' 		=> $rev,
			);
