                                            ...
                                            <div class="col-md-6">
                                                <div class="btn-group">
                                                  <?php
                                                    if($priv_add == 1)
                                                    {
                                                      echo '<button id="modalAdd-btn" class="btn sbold dark" data-toggle="modal" onclick="openFormMatauang(),reset()"><i class="icon-plus"></i>&nbsp; Tambah Data
                                                      </button>';
                                                    }
                                                  ?>
        ...
        <script type="text/javascript">
            ...
            function searchData() { 
                $('#default-table').DataTable({
                    ...
                    "serverSide": true,
                    ajax: {
                      url: '<?php echo base_url();?>Master-Data/Mata-Uang/loadData/'
                    },
                    "columns": [
                      {"name": "no","orderable": false,"searchable": false,  "className": "text-center", "width": "5%"},
                      {"name": "mata_uang_nama"},
                      {"name": "mata_uang_status_aktif"},
              ...
              function editData(id) {
                $.ajax({
                  type : "GET",
                  url  : '<?php echo base_url();?>Master-Data/Mata-Uang/loadDataWhere/',
                  data : "id="+id,
                  dataType : "json",
                  success:function(data){
                    for(var i=0; i<data.val.length;i++){
                      document.getElementsByName("kode")[0].value = data.val[i].kode;
                      document.getElementsByName("mata_uang_nama")[0].value = data.val[i].mata_uang_nama;
                      if (data.val[i].mata_uang_status_aktif == 'y') {
                        document.getElementById('aktif').selected = true;
                      } else if (data.val[i].mata_uang_status_aktif == 'n') {
              ...
              function deleteData(id) {
                swal({
                  ...
                  $.ajax({
                    url: '<?php echo base_url();?>Master-Data/Mata-Uang/deleteData/',
                    data: 'id='+id,
                    type: 'POST',
                    dataType: 'json',
                    success: function (data) {
              ...
              function aktifData(id) {
                swal({
                  ...
                  $.ajax({
                    url: '<?php echo base_url();?>Master-Data/Mata-Uang/aktifData/',
                    data: 'id='+id,
                    type: 'POST',
                    dataType: 'json',
                    success: function (data) {
