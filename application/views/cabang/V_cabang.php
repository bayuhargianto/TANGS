                                            ...
                                            <div class="col-md-6">
                                                <div class="btn-group">
                                                  <?php
                                                    if($priv_add == 1)
                                                    {
                                                      echo '<button id="modalAdd-btn" class="btn sbold dark" data-toggle="modal" onclick="openFormCabang(),reset()"><i class="icon-plus"></i>&nbsp; Tambah Data
                                                      </button>';
                                                    }
                                                  ?>
        ...
        <script type="text/javascript">
            ...
            function searchData() {
                $('#default-table').DataTable({
                    ...
                    "serverSide": true,
                    ajax: {
                      url: '<?php echo base_url();?>Master-Data/Cabang/loadData/'
                    },
                    "columns": [
                      {"name": "no","orderable": false,"searchable": false,  "className": "text-center", "width": "5%"},
                      {"name": "cabang_nama"},
                      {"name": "cabang_status_aktif"},
              ...
              function editData(id) {
                $.ajax({
                  type : "GET",
                  url  : '<?php echo base_url();?>Master-Data/Cabang/loadDataWhere/',
                  data : "id="+id,
                  dataType : "json",
                  success:function(data){
                    for(var i=0; i<data.val.length;i++){
                      ...
                      for (var j = 2; j <= itemOption; j++) {
                          generateItemOption2(j, "cabang_telepon", "default_value2", "detail");
                      }
                      for(var j=0; j<data.val[i].cabang_telepon.val2.length; j++){
                        document.getElementById("cabang_telepon"+(j+1)).value = data.val[i].cabang_telepon.val2[j].text
                      }
                      // fax
                      ...
                      for (var j = 2; j <= itemOptionFax; j++) {
                          generateItemOption2(j, "cabang_fax", "default_value2Fax", "detailFax");
                      }
                      for(var j=0; j<data.val[i].cabang_fax.val2.length; j++){
                        document.getElementById("cabang_fax"+(j+1)).value = data.val[i].cabang_fax.val2[j].text
                      }
                      // end fax
                      // email
                      ...
                      for (var j = 2; j <= itemOptionEmail; j++) {
                          generateItemOption2(j, "cabang_email", "default_value2Email", "detailEmail");
                      }
                      for(var j=0; j<data.val[i].cabang_email.val2.length; j++){
                        document.getElementById("cabang_email"+(j+1)).value = data.val[i].cabang_email.val2[j].text
                      }
                      // end email
                      // document.getElementsByName("cabang_fax")[0].value = data.val[i].cabang_fax;
                      // document.getElementsByName("cabang_email")[0].value = data.val[i].cabang_email;
                      // document.getElementsByName("cabang_email")[0].value = data.val[i].cabang_email;
                      if (data.val[i].cabang_status_aktif == 'y') {
                        document.getElementById('aktif').selected = true;
                      } else if (data.val[i].cabang_status_aktif == 'n') {
              ...
              function deleteData(id) {
                swal({
                  ...
                  $.ajax({
                    url: '<?php echo base_url();?>Master-Data/Cabang/deleteData/',
                    data: 'id='+id,
                    type: 'POST',
                    dataType: 'json',
                    success: function (data) {
              ...
              function aktifData(id) {
                swal({
                  ...
                  $.ajax({
                    url: '<?php echo base_url();?>Master-Data/Cabang/aktifData/',
                    data: 'id='+id,
                    type: 'POST',
                    dataType: 'json',
                    success: function (data) {
