                                            ...
                                            <div class="col-md-6">
                                                <div class="btn-group">
                                                  <?php
                                                    if($priv_add == 1)
                                                    {
                                                      echo '<button id="modalAdd-btn" class="btn sbold dark" data-toggle="modal" onclick="openFormBrand(),reset()"><i class="icon-plus"></i>&nbsp; Tambah Data
                                                      </button>';
                                                    }
                                                  ?>
        ...
        <script type="text/javascript">
            ...
            function searchData() { 
                $('#default-table').DataTable({
                    ...
                    "serverSide": true,
                    ajax: {
                      url: '<?php echo base_url();?>Master-Data/Master-Brand/loadData/'
                    },
                    "columns": [
                      {"name": "no","orderable": false,"searchable": false,  "className": "text-center", "width": "5%"},
                      {"name": "brand_nama"},
                      {"name": "brand_status_aktif"},
              ...
              function editData(id) {
                $.ajax({
                  type : "GET",
                  url  : '<?php echo base_url();?>Master-Data/Master-Brand/loadDataWhere/',
                  data : "id="+id,
                  dataType : "json",
                  success:function(data){
                    for(var i=0; i<data.val.length;i++){
                      document.getElementsByName("kode")[0].value = data.val[i].kode;
                      document.getElementsByName("brand_nama")[0].value = data.val[i].brand_nama;
                      if (data.val[i].brand_status_aktif == 'y') {
                        document.getElementById('aktif').selected = true;
                      } else if (data.val[i].brand_status_aktif == 'n') {
              ...
              function deleteData(id) {
                swal({
                  ...
                  $.ajax({
                    url: '<?php echo base_url();?>Master-Data/Master-Brand/deleteData/',
                    data: 'id='+id,
                    type: 'POST',
                    dataType: 'json',
                    success: function (data) {
              ...
              function aktifData(id) {
                swal({
                  ...
                  $.ajax({
                    url: '<?php echo base_url();?>Master-Data/Master-Brand/aktifData/',
                    data: 'id='+id,
                    type: 'POST',
                    dataType: 'json',
                    success: function (data) {
