                                            ...
                                            <div class="col-md-6">
                                                <div class="btn-group">
                                                  <?php
                                                    if($priv_add == 1)
                                                    {
                                                      echo '<button id="modalAdd-btn" class="btn sbold dark" data-toggle="modal" onclick="openFormKaryawan(),reset()"><i class="icon-plus"></i>&nbsp; Tambah Data
                                                      </button>';
                                                    }
                                                  ?>
        ...
        <script type="text/javascript">
            ...
            function searchData() { 
                $('#default-table').DataTable({
                    ...
                    "serverSide": true,
                    ajax: {
                      url: '<?php echo base_url();?>Master-Data/Karyawan/loadData/'
                    },
                    "columns": [
              ...
              function editData(id) {
                $.ajax({
                  type : "GET",
                  url  : '<?php echo base_url();?>Master-Data/Karyawan/loadDataWhere/',
                  data : "id="+id,
                  dataType : "json",
                  success:function(data){
                    for(var i=0; i<data.val.length;i++){
                      document.getElementsByName("kode")[0].value = data.val[i].kode;
                      document.getElementsByName("karyawan_nip")[0].value = data.val[i].karyawan_nip;
                      document.getElementsByName("karyawan_nama")[0].value = data.val[i].karyawan_nama;
                      document.getElementsByName("karyawan_alamat")[0].value = data.val[i].karyawan_alamat;
                      for(var j=0; j<data.val[i].m_type_karyawan_id.val2.length; j++){
                        $("#m_type_karyawan_id").append('<option value="'+data.val[i].m_type_karyawan_id.val2[j].id+'" selected>'+data.val[i].m_type_karyawan_id.val2[j].text+'</option>');
                      }
                      ...
                      for (var j = 2; j <= itemOption; j++) {
                          generateItemOption2(j);
                      }
                      for(var j=0; j<data.val[i].karyawan_telepon.val2.length; j++){
                        document.getElementById("karyawan_telepon"+(j+1)).value = data.val[i].karyawan_telepon.val2[j].text
                      }
                      if (data.val[i].karyawan_status_aktif == 'y') {
                        document.getElementById('aktif').selected = true;
                      } else if (data.val[i].karyawan_status_aktif == 'n') {
              ...
              function deleteData(id) {
                swal({
                  ...
                  $.ajax({
                    url: '<?php echo base_url();?>Master-Data/Karyawan/deleteData/',
                    data: 'id='+id,
                    type: 'POST',
                    dataType: 'json',
                    success: function (data) {
              ...
              function aktifData(id) {
                swal({
                  ...
                  $.ajax({
                    url: '<?php echo base_url();?>Master-Data/Karyawan/aktifData/',
                    data: 'id='+id,
                    type: 'POST',
                    dataType: 'json',
                    success: function (data) {
