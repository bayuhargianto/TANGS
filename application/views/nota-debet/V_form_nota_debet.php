                            ...
                            <div class="portlet light portlet-fit portlet-datatable bordered">
                                ...
                                <form action="#" id="formAdd" class="form-horizontal" method="post">
                                    <div class="form-body">
                                        ...
                                        <input type="hidden" id="url" value="Pembelian/Nota-Debet/postData/">
                                        <input type="hidden" id="url_data" value="Pembelian/Nota-Debet">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="col-md-8">
                                            <div class="input-icon right">
                                                <i class="fa"></i>
                                                <select class="form-control select2" name="nota_debet_metode_pembayaran" aria-required="true" aria-describedby="select-error" required>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="col-md-8">
                                            <div class="input-icon right">
                                                <i class="fa"></i>
                                                <textarea class="form-control" rows="3" name="nota_debet_catatan"></textarea> </div>
                                        </div>
                                    </div>
                                    <div class="form-actions">
                                        <div class="row">
                                            <div class="col-md-offset-4 col-md-8 text-right">
                                                ...
                                                <a href="<?php echo base_url();?>Pembelian/Nota-Debet">
        ...
        <script type="text/javascript">
            ...
            function addRetur() {
                var id = document.getElementsByName('t_retur_pembelian_id')[0].value;
                if (id.length > 0) {
                    getDetailBarang(id);
                }
            }
                ...
                $.ajax({
                  ...
                  success:function(data){
                    for(var i = 0; i < data.val.length; i++){
                        ...
                        document.getElementsByName('t_penerimaan_barang_id')[0].value = data.val[i].penerimaan_barang_nomor.val2[0].id;
                        getDetailPenerimaan(data.val[i].penerimaan_barang_nomor.val2[0].id);
                    }
                    ...
                    for(var i = 0; i < data.val2.length; i++){
                        document.getElementsByName('nota_debet_ppn')[0].value = data.val2[i].penerimaan_barang_ppn;
                        $("#default-table tbody").append('\
                            <tr id="detail'+(i+1)+'">\
                                ...
                                <td id="td2'+(i+1)+'">\
                                    <input type="hidden" name="t_retur_pembeliandet_id[]" value="'+data.val2[i].retur_pembeliandet_id+'"/>\
                                ...
                                <td id="td4'+(i+1)+'">\
                                    <input type="text" class="form-control" id="nota_debetdet_qty'+(i+1)+'" name="nota_debetdet_qty[]" value="'+data.val2[i].retur_pembeliandet_qty+'" required readonly/>\
                                </td>\
                                <td id="td5'+(i+1)+'">\
                                    '+data.val2[i].satuan_nama+'\
                                </td>\
                                <td id="td6'+(i+1)+'">\
                                    <input type="text" class="form-control money" id="nota_debetdet_harga_satuan'+(i+1)+'" name="nota_debetdet_harga_satuan[]" value="'+data.val2[i].penerimaan_barangdet_harga_satuan+'" onchange="sumSubTotal()" required/>\
                                </td>\
                                <td id="td7'+(i+1)+'">\
                                    <input type="text" class="form-control money" id="nota_debetdet_potongan'+(i+1)+'" name="nota_debetdet_potongan[]" value="'+data.val2[i].penerimaan_barangdet_potongan+'" onchange="sumSubTotal()" required/>\
                                </td>\
                                <td id="td8'+(i+1)+'">\
                                    <input type="text" class="form-control money" id="nota_debetdet_total'+(i+1)+'" name="nota_debetdet_total[]" value="" readonly required/>\
                                </td>\
                            </tr>\
                        ');
                    }
            ...
            function sumSubTotal() {
                ...
                for (var i = 1; i <= itemBarang; i++) {
                    qty = document.getElementById('nota_debetdet_qty'+i).value;
                    hrg = parseInt(document.getElementById('nota_debetdet_harga_satuan'+i).value.replace(/\./g, ""));
                    pot = parseInt(document.getElementById('nota_debetdet_potongan'+i).value.replace(/\./g, ""));
                    document.getElementById('nota_debetdet_total'+i).value = qty * hrg - pot;
                    subTotal += qty * hrg - pot;
                }
                document.getElementById('nota_debet_subtotal').value = subTotal;
                $('.money').number( true, 2, '.', ',' );
                sumTotal();
            }

            function sumTotal() {
                subTotal = parseInt(document.getElementById('nota_debet_subtotal').value.replace(/\./g, ""));
                ppn = parseInt(document.getElementById('nota_debet_ppn').value.replace(/\./g, ""));
                document.getElementById('nota_debet_total').value = subTotal + (subTotal * ppn / 100);
                $('.money').number( true, 2, '.', ',' );
            }
            ...
            function getDetailPenerimaan(id) {
                $.ajax({
                  ...
                  success:function(data){
                    for(var i = 0; i < data.val.length; i++){
                      document.getElementsByName('t_order_id')[0].value = data.val[i].t_order_id.val2[0].id;
                      getDetailPembelian(data.val[i].t_order_id.val2[0].id);
                      document.getElementsByName('m_gudang_id')[0].value = data.val[i].m_gudang_id.val2[0].id;
                      document.getElementsByName('gudang_nama')[0].value = data.val[i].m_gudang_id.val2[0].text;
                    }
                  }
                });
            }
            ...
            function getDetailPembelian(id) {
                $.ajax({
                  ...
                  success:function(data){
                    for(var i = 0; i < data.val.length; i++){
                      document.getElementsByName('m_partner_id')[0].value = data.val[i].m_supplier_id.val2[0].id;
                      getDetailSupplier(data.val[i].m_supplier_id.val2[0].id);
                      document.getElementsByName('partner_nama')[0].value = data.val[i].m_supplier_id.val2[0].text;
                    }
                  }
                });
            }
