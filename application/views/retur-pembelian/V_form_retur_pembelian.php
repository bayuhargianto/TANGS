                            ...
                            <div class="portlet light portlet-fit portlet-datatable bordered">
                                ...
                                <form action="#" id="formAdd" class="form-horizontal" method="post">
                                    <div class="form-body">
                                        ...
                                        <input type="hidden" id="url" value="Gudang/Retur-Pembelian/postData/">
                                        <input type="hidden" id="url_data" value="Gudang/Retur-Pembelian">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="col-md-8">
                                                <div class="input-icon right">
                                                    <i class="fa"></i>
                                                    <select class="form-control" id="t_penerimaan_barang_id" name="t_penerimaan_barang_id" aria-required="true" aria-describedby="select-error" onchange="addBPB()">
                                                    </select>
                                                </div>
                                            </div>
                                            <!-- <div class="col-md-1">
                                                <button type="button" id="btnAddBPB" class="btn sbold dark" onclick="addBPB()"><i class="icon-plus"></i></button>
                                            </div> -->
                                        </div>
                                    ...
                                    <div class="form-actions">
                                        <div class="row">
                                            <div class="col-md-offset-4 col-md-8 text-right">
                                                ...
                                                <a href="<?php echo base_url();?>Gudang/Retur-Pembelian">
        ...
        <script type="text/javascript">
            ...
            function addBPB() {
                var id = document.getElementsByName('t_penerimaan_barang_id')[0].value;
                if (id.length > 0) {
                    getDetailBarang(id);
                }
            }
            ...
            function addBarang() {
                var id = document.getElementsByName('t_penerimaan_barang_id')[0].value;
                var id2 = document.getElementsByName('m_barang_id')[0].value;
                if (id.length > 0) {
                    getDetailBarang2(id, id2);
                }
            }

            function removeBPB(itemSeq) {
                ...
                $.ajax({
                  ...
                  success:function(data){
                    ...
                    for(var i = 0; i < data.val2.length; i++){
                        $("#m_barang_id").append('<option value="'+data.val2[i].penerimaan_barangdet_id+'">'+data.val2[i].barang_kode+' - '+data.val2[i].jenis_barang_nama+'</option>');
                        ...
                        //         </td>\
                        //         <td id="td2'+(i+1)+'">\
                        //             <input type="hidden" name="t_penerimaan_barangdet_id[]" value="'+data.val2[i].penerimaan_barangdet_id+'"/>\
                        //             <input type="hidden" name="m_barang_id[]" value="'+data.val2[i].m_barang_id+'"/>\
                        //             <input type="hidden" name="retur_pembeliandet_id[]" value=""/>\
                        ...
                        //         </td>\
                        //         <td id="td3'+(i+1)+'">\
                        //             <input type="text" class="form-control" id="retur_pembeliandet_qty_terima'+(i+1)+'" name="retur_pembeliandet_qty_terima[]" value="'+(parseInt(data.val2[i].penerimaan_barangdet_qty) - parseInt(data.val2[i].penerimaan_barangdet_qty_retur))+'" required readonly/>\
                        //         </td>\
                        //         <td id="td3'+(i+1)+'">\
                        //             <input type="text" class="form-control" id="retur_pembeliandet_qty'+(i+1)+'" name="retur_pembeliandet_qty[]" value="'+(parseInt(data.val2[i].penerimaan_barangdet_qty) - parseInt(data.val2[i].penerimaan_barangdet_qty_retur))+'" onchange="checkQty('+(i+1)+')" required/>\
                        //         </td>\
                        //         <td id="td5'+(i+1)+'">\
                        //             '+data.val2[i].satuan_nama+'\
                        //         </td>\
                        //         <td id="td6'+(i+1)+'">\
                        //             <textarea class="form-control" rows="3" name="retur_pembeliandet_keterangan[]"></textarea>\
                        //         </td>\
                        //         <td id="td7'+(i+1)+'">\
                        //             <button type="button" id="removeBtn" class="btn red-thunderbird" onclick="removeBPB('+(i+1)+')">\
            ...
            function getDetailBarang2(id,id2) {
                $.ajax({
                  type : "GET",
                  url  : '<?php echo base_url();?>Gudang/Penerimaan-Barang/loadDataWhere/',
                  data : { id : id },
                  ...
                  success:function(data){
                    for(var i = 0; i < data.val2.length; i++){
                        if (data.val2[i].penerimaan_barangdet_id == id2) {
                            itemBarang++;
                            $("#default-table tbody").append('\
                                <tr id="detail'+itemBarang+'">\
                                    ...
                                    <td id="td4'+itemBarang+'">\
                                        <input type="text" class="form-control num2" id="retur_pembeliandet_qty_terima'+itemBarang+'" name="retur_pembeliandet_qty_terima[]" value="'+(parseInt(data.val2[i].penerimaan_barangdet_qty) - parseInt(data.val2[i].penerimaan_barangdet_qty_retur))+'" required readonly/>\
                                    </td>\
                                    <td id="td5'+itemBarang+'">\
                                        <input type="text" class="form-control num2" id="retur_pembeliandet_qty'+itemBarang+'" name="retur_pembeliandet_qty[]" value="'+(parseInt(data.val2[i].penerimaan_barangdet_qty) - parseInt(data.val2[i].penerimaan_barangdet_qty_retur))+'" onchange="checkQty('+itemBarang+')" required/>\
                                    </td>\
                                    ...
                                    <td id="td8'+itemBarang+'">\
                                        <button type="button" id="removeBtn" class="btn red-thunderbird" onclick="removeBPB('+itemBarang+')">\
                                            <i class="icon-close"></i>\
                                        </button>\
                                    </td>\
                                </tr>\
            ...
            function checkQty(idx) {
                qty1 = document.getElementById("retur_pembeliandet_qty_terima"+idx);
                qty2 = document.getElementById("retur_pembeliandet_qty"+idx);
                if (parseInt(qty2.value) > parseInt(qty1.value)) {
                    swal({
                        title: "Alert!",
                        text: "Jumlah retur melebihi jumlah penerimaan!",
                        type: "error",
            ...
            function editData(id) {
                $.ajax({
                  type : "GET",
                  url  : '<?php echo base_url();?>Gudang/Retur-Pembelian/loadDataWhere/',
                  data : "id="+id,
                  dataType : "json",
                  success:function(data){
                    for(var i=0; i<data.val.length;i++){
                    ...
                    for(var i = 0; i < data.val2.length; i++){
                        $("#default-table tbody").append('\
                            <tr id="detail'+(i+1)+'">\
                                ...
                                <td id="td7'+(i+1)+'">\
                                    <button type="button" id="removeBtn" class="btn red-thunderbird" onclick="removeBPB('+(i+1)+')" disabled>\
                                        <i class="icon-close"></i>\
                                    </button>\
                                </td>\
                            </tr>\
