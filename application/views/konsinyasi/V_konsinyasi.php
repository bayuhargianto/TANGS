                                            ...
                                            <div class="col-md-6">
                                                <div class="btn-group">
                                                  <?php
                                                    if($priv_add == 1)
                                                    {
                                                      echo '<button id="modalAdd-btn" class="btn sbold dark" data-toggle="modal" onclick="openFormKonsinyasi(),reset()"><i class="icon-plus"></i>&nbsp; Tambah Data
                                                      </button>';
                                                    }
                                                  ?>
        ...
        <script type="text/javascript">
            ...
            function searchData() { 
                $('#default-table').DataTable({
                    ...
                    "serverSide": true,
                    ajax: {
                      url: '<?php echo base_url();?>Master-Data/Konsinyasi/loadData/'
                    },
                    "columns": [
                      {"name": "no","orderable": false,"searchable": false,  "className": "text-center", "width": "5%"},
                      {"name": "barang_nama"},
                      {"name": "m_jenis_barang_id"},
                      {"name": "m_category_2_id"},
                      {"name": "konsinyasi_status_aktif"},
              ...
              function editData(id) {
                $.ajax({
                  type : "GET",
                  url  : '<?php echo base_url();?>Master-Data/Konsinyasi/loadDataWhere/',
                  data : "id="+id,
                  dataType : "json",
                  success:function(data){
                    for(var i=0; i<data.val.length;i++){
                      ...
                      if (data.val[i].konsinyasi_status_aktif == 'y') {
                        document.getElementById('aktif').selected = true;
                      } else if (data.val[i].konsinyasi_status_aktif == 'n') {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                            if (data > 0) {
                              ...
                              swal({
                                title: "Warning",
                                text: "Data Tidak Bisa Dihapus Karena Stok Masih Belum Kosong !",
                                type: "warning",
                                confirmButtonClass: "btn-raised btn-danger",
                                confirmButtonText: "OK"
                                })
                              // alert("DATA TIDAK BISA DIHAPUS KARENA STOK BELUM KOSONG !");
                            } else {
                              // alert(data);
                                swal({
                                ...
                                closeOnConfirm: false
                                  ...
                                  $.ajax({
                                    url: '<?php echo base_url();?>Master-Data/Konsinyasi/deleteData/',
                                    data: 'id='+id,
                                    type: 'POST',
                                    dataType: 'json',
                                    success: function (data) {
              ...
              function aktifData(id) {
                swal({
                  ...
                  $.ajax({
                    url: '<?php echo base_url();?>Master-Data/Konsinyasi/aktifData/',
                    data: 'id='+id,
                    type: 'POST',
                    dataType: 'json',
                    success: function (data) {
